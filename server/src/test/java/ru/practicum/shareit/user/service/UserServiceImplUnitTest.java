...
class UserServiceImplUnitTest {
    ...
    @Test
    void update_whenDuplicateEmail_thenDuplicateEntityExceptionThrown() {
        Long userId = 1L;
        Long otherUserId = 2L;
        User otherUser = User.builder()
                .id(2L)
                .name("oldName")
                .email("sameEmail@test.ru")
                .build();
        User savedUser = User.builder()
                .id(1L)
                .name("oldName")
        ...
        when(userRepository.findById(userId))
                .thenReturn(Optional.of(savedUser));
        when(userRepository.findByEmail(userToUpdate.getEmail()))
                .thenReturn(Optional.of(otherUser));

        assertThrows(DuplicateEntityException.class,
                () -> userService.update(userId, userToUpdate),
                "Updated email already save with another user, but no DuplicateEntityException thrown");
    }
